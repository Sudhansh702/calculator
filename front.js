...
function appendOperator(o){
    if ((exp.length!=0) && ((exp.charAt(exp.length-1) == '+') || (exp.charAt(exp.length-1) == '-') || (exp.charAt(exp.length-1) == '*') || (exp.charAt(exp.length-1) == '/'))) {
        exp.charAt(exp.length-1) = o
        result.innerHTML = exp
    }else{
        exp += o
        result.innerHTML = exp
...
function calculateResult(){
    const wordsArray = [...infixToPostfix(exp) ];
    console.log(wordsArray)
    result.innerHTML= calcEquation(wordsArray)
    exp = ""
}
function reset(){
    result.innerHTML =0
...
// let xyz = 
// result.innerHTML = calcEquation(xyz)